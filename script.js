document.addEventListener("DOMContentLoaded", () => {
  ...
  function initDynamicSidebar() {
    ...
    const contentTitles = {
      ...
      "tendencias-mercado": "Tendências de Mercado - Sebrae Nacional",
  ...
  function showStepContent(stepNumber) {
    ...
    // Dentro da função showStepContent, após as outras condições, adicionar:
    if (stepNumber === "2") {
      initStep2Functionality()
    }
  }
  ...
  */
  ...
  // Inicializa a funcionalidade se a Etapa 6 já estiver ativa
  if (
  ...
  // Adicionar a função initStep2Functionality antes do final do arquivo:

  // Funcionalidade da Etapa 2 - Diretrizes e Orientações Estratégicas
  function initStep2Functionality() {
    // Funcionalidade do menu lateral da Etapa 2
    const step2SidebarItems = document.querySelectorAll("#step-content-2 .sidebar-section li[data-content]")
    const step2ContentTitle = document.getElementById("content-title-2")
    const step2DynamicContents = document.querySelectorAll("#step-content-2 .dynamic-content")

    // Mapeamento de conteúdos para títulos da Etapa 2
    const step2ContentTitles = {
      "cronograma-elaboracao": "Cronograma de Elaboração",
      "cidades-territorios": "Cidades e Territórios Empreendedores",
      "conecta-tech": "Conecta Tech",
      "integridade-privacidade": "Integridade e Privacidade",
      "gestao-marca": "Gestão da Marca",
      "documentos-anos-anteriores": "Documentos de Anos Anteriores",
      "equipes-estado": "Equipes por Estado",
    }

    function showStep2Content(contentId) {
      // Esconde todos os conteúdos da Etapa 2
      step2DynamicContents.forEach((content) => {
        content.classList.remove("active")
      })
      ...
      // Atualiza o título
      if (step2ContentTitle && step2ContentTitles[contentId]) {
        step2ContentTitle.textContent = step2ContentTitles[contentId]
      }
    }

    // Adiciona event listeners aos itens do menu da Etapa 2
    step2SidebarItems.forEach((item) => {
      item.addEventListener("click", function () {
        const contentId = this.getAttribute("data-content")
        ...
        // Mostra o conteúdo correspondente
        showStep2Content(contentId)
      })
    })

    // Garantir que o primeiro item esteja ativo e visível
    const firstSidebarItem = step2SidebarItems[0]
    if (firstSidebarItem) {
      firstSidebarItem.classList.add("active")
      const firstContentId = firstSidebarItem.getAttribute("data-content")
      showStep2Content(firstContentId)
    }

    // Funcionalidade para itens em negrito (clicáveis)
    const boldItems = document.querySelectorAll("#step-content-2 .bold-item")
    boldItems.forEach((item) => {
      item.addEventListener("click", function (e) {
        e.stopPropagation()

        const parentLi = this.closest("li")
        const contentId = parentLi.getAttribute("data-content")

        if (contentId) {
          // Remove active de todos os itens
          const allItems = document.querySelectorAll("#step-content-2 .sidebar-section li")
  ...
  // Funcionalidade das abas para Equipes por Estado
  function initTabsEquipesEstado() {
    ...
    tabButtons.forEach((button) => {
      button.addEventListener("click", () => {
        ...
        if (tabName === "equipes") {
          document.getElementById("tab-equipes").classList.add("active")
        } else if (tabName === "atendimento") {
          document.getElementById("tab-atendimento").classList.add("active")
        }
      })
    })
  }

  // Atualizar a função showStepContent para incluir as novas seções
  const originalShowStepContent4 = showStepContent
  showStepContent = (stepNumber) => {
    originalShowStepContent4(stepNumber)
    ...
    setTimeout(() => {
      if (stepNumber === "4") {
        ...
        initStep2Functionality()
        // Inicializar abas se estiver na seção de equipes por estado
        setTimeout(initTabsEquipesEstado, 200)
      } else if (stepNumber === "1") {
        // Reativar funcionalidade da Etapa 1
        initDynamicSidebar()
        // Garantir que o primeiro item esteja ativo
  ...
  // Inicializar abas se já estiver na Etapa 2
  if (
    document.getElementById("step-content-2") &&
    document.getElementById("step-content-2").classList.contains("active")
  ) {
    setTimeout(initTabsEquipesEstado, 200)
  }
})
